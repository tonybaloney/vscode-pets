...
export function registerChatHandler(extensionContext: vscode.ExtensionContext) {
    const petsChat = vscode.chat.createChatParticipant(
        'vscode-pets.pets',
        async (
            ...
            try {
                var petList = PetSpecification.collectionFromMemento(
                    ...
                    .map(
                        (pet) =>
                            `- Name: "${pet.name}", Animal: ${pet.type}, Color: ${pet.color}`,
                    )
                    .join('\n ');
                    ...
                    if (fragment instanceof vscode.LanguageModelTextPart) {
                        ...
                        if (firstToken && fragment.value.startsWith('<')) {
                            ...
                            if (fragment.value.includes('>')) {
                                writingMeta = false;
                                petDetails += fragment.value.substring(
                                    0,
                                    fragment.value.indexOf('>') - 1,
                                );
                                const petDetailsArray = petDetails
                                    .split('|')
                                    .map((part) => part.trim());
                                petSelection = {
                                    petName: petDetailsArray[0].trim(),
                                    petType: petDetailsArray[1]
                                        .toLowerCase()
                                        .trim(),
                                    petColor: petDetailsArray[2]
                                        .toLowerCase()
                                        .trim(),
                                };
                                // Send anything after the > to the stream
                                stream.markdown(
                                    fragment.value.substring(
                                        fragment.value.indexOf('>') + 1,
                                    ),
                                );
                            } else {
                ...
                if (petSelection) {
                    stream.button({
                        ...
                        title: vscode.l10n.t(
                            `Thank ${petSelection.petName} the ${petSelection.petType} for their help!`,
                        ),
                        arguments: [
    ...
    petsChat.iconPath = vscode.Uri.joinPath(
